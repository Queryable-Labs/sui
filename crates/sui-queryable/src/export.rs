use crate::types::{
    ...
    ENTITY_FIELD_TX_INDEX, ENTITY_TRANSACTIONS_NAME,
};
...
impl QueryableExporter {
        ...
        let entities = vec![
            Entity::new(
                ...
                vec![
                    ...
                    EntityField::create_field(
                        String::from("gas_input_id"),
                        EntityFieldType::Binary(true),
                    ),
                    EntityField::create_field(
                        String::from("gas_input_version"),
                        EntityFieldType::Uint64(true),
                    ),
                    EntityField::create_field(
                        String::from("gas_input_digest"),
                        EntityFieldType::Binary(true),
                    ),
                    EntityField::create_field(
    ...
    pub fn add_transaction(
        ...
        debug!("Adding transaction seq {}", seq);
        ...
        transaction_writer.add_value_binary(
            String::from("gas_input_id"),
            Some(cert.gas_payment_object_ref().0.to_vec()),
        )?;

        transaction_writer.add_value_u64(
            String::from("gas_input_version"),
            Some(cert.gas_payment_object_ref().1.value()),
        )?;

        transaction_writer.add_value_binary(
            String::from("gas_input_digest"),
            Some(cert.gas_payment_object_ref().2 .0.to_vec()),
        )?;
